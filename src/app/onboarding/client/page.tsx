          ...
          {!user && (
            <div className="mt-6 p-5 bg-pingwash-ice rounded-2xl border border-pingwash-blue/20">
              ...
              <button
                type="button"
                onClick={async () => {
                  // Store booking data before redirecting to Google
                  sessionStorage.setItem("pingwash_pending_booking", JSON.stringify({
                    vehicleType, vehicleBrand, vehicleModel,
                    locationType, address, addressComplement, postalCode, city,
                    scheduleType, selectedDate, selectedTime,
                    selectedForfait, selectedOptions, total,
                  }));
                  await supabase.auth.signInWithOAuth({
                    provider: "google",
                    options: {
                      redirectTo: `${window.location.origin}/auth/callback?redirect=/onboarding/client`,
                    },
                  });
                }}
                className="flex items-center w-full py-3 px-4 rounded-xl border border-gray-200 hover:bg-white transition-colors bg-white mb-4"
              >
